...
public:
    ListNode* mergeKLists(vector<ListNode*>& lists){
        ...
        while(!pq.empty()){
            ...
            if(pr.second->next) pq.push({pr.second->next->val, pr.second->next}); //if not empty, add the next node from the list
                                                                                  //where we had taken the node we just added
            temp->next = pr.second; //add the pair address to dummy 
            temp = temp->next; //move to that new node
        }
        return dummy.next;
    }
};
